...
class Application_M extends CI_Model {
	...
	public function doActivation($params, $student) {
		$user_id = 0;
		...
		if ($query->num_rows() > 0) {
			...
			$params['basic']['account_type'] = STUDENT;
			$params['basic']['status'] = ACTIVATED;
		...
		$this->db_conn->where("student_id = {$student_id}")
					->update('students', array(	'user_id' => $user_id, 
												'status' => ACTIVATED, 
												'start_time' => $_SERVER['REQUEST_TIME'], 
												'end_time' => $_SERVER['REQUEST_TIME'] + 86400 * $student['duration']));
